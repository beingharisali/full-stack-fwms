...
const createDriver = async (req, res) => {
	try {
		const { firstName, lastName, licenseNumber, licenseType, email, password } = req.body;

		if (!firstName || !lastName || !licenseNumber || !licenseType) {
			return res.status(400).json({
				success: false,
				msg: "Please provide first name, last name, license number, and license type",
			});
		}
		...
		const driverData = {
			firstName,
			lastName,
			name: `${firstName} ${lastName}`, // 🔹 Ensure full name
			licenseNumber,
			licenseType,
			user: createdUser ? createdUser._id : null, // link driver to user
		};
		...
		res.status(500).json({ success: false, msg: "Error creating driver", error: error.message });
	}
};
...
const getAllDrivers = async (req, res) => {
	try {
		...
		res.status(500).json({ success: false, msg: "Error fetching drivers", error: error.message });
	}
};
...
const getDriverById = async (req, res) => {
	try {
		...
		res.status(400).json({ success: false, msg: "Invalid driver ID", error: error.message });
	}
};
...
const getDriverByUserId = async (req, res) => {
	try {
		...
		if (!driver) return res.status(404).json({ success: false, msg: "No driver profile found for this user" });
		res.status(200).json({ success: true, driver });
	} catch (error) {
		res.status(400).json({ success: false, msg: "Error fetching driver profile", error: error.message });
	}
};
...
const updateDriver = async (req, res) => {
	try {
		...
		res.status(400).json({ success: false, msg: "Error updating driver", error: error.message });
	}
};
...
const deleteDriver = async (req, res) => {
	try {
		...
		res.status(400).json({ success: false, msg: "Error deleting driver", error: error.message });
	}
};
...
const assignVehicleToDriver = async (req, res) => {
	try {
		...
		if (driver.assignedVehicle || vehicle.assignedTo) return res.status(400).json({ success: false, msg: "Driver or Vehicle already assigned" });
		...
		res.status(500).json({ success: false, msg: "Error assigning vehicle", error: error.message });
	}
};
...
const unassignVehicleFromDriver = async (req, res) => {
	try {
		...
		if (!driver || !driver.assignedVehicle) return res.status(400).json({ success: false, msg: "No vehicle assigned to this driver" });
		...
		res.status(500).json({ success: false, msg: "Error unassigning vehicle", error: error.message });
	}
};
...
const assignedVsFreeDrivers = async (req, res) => {
	try {
		...
		res.status(500).json({ success: false, msg: "Error fetching assigned/free report" });
	}
};
...
const driversByLicenseType = async (req, res) => {
	try {
		...
		res.status(500).json({ success: false, msg: "Error fetching license type report" });
	}
};
